//
//  CustomEmojiExplorerViewController.swift
//  LearnApp
//
...
class CustomEmojiExplorerViewController: UIViewController {
...
extension CustomEmojiExplorerViewController {
    ...
    // custom
    func createCustomHeaderCellRegistration() -> UICollectionView.CellRegistration<CustomHeaderCell, String> {
        return UICollectionView.CellRegistration<CustomHeaderCell, String> { (cell, indexPath, title) in
            let view = UIView()
            view.backgroundColor = .red
            let customAccessory = UICellAccessory.CustomViewConfiguration(customView: view, placement: .trailing(displayed: .always))
            ...
            cell.accessories = [.outlineDisclosure(displayed: .always, options: .init(style: .header))]
        }
    }

    func createCustomRegularCellRegistration() -> UICollectionView.CellRegistration<CustomRegularCell, Emoji> {
        return UICollectionView.CellRegistration<CustomRegularCell, Emoji> { (cell, indexPath, emoji) in
    ...
    func configureDataSource() {
        ...
        let customHeaderCellRegistration = createCustomHeaderCellRegistration()
        let customRegularCellRegistration = createCustomRegularCellRegistration()
        ...
        dataSource = UICollectionViewDiffableDataSource<Section, Item>(collectionView: collectionView) { (collectionView, indexPath, item) -> UICollectionViewCell? in
            ...
            case .outline:
                if item.hasChildren {
                    let cell = collectionView.dequeueConfiguredReusableCell(using: customHeaderCellRegistration, for: indexPath, item: item.title!)
                   
                        // Configure the disclosure indicator based on the expansion state
                    cell.setDisclosureIndicator(isExpanded: false) // Adjust based on your logic
                    
                    return cell
                } else {
                    return collectionView.dequeueConfiguredReusableCell(using: customRegularCellRegistration, for: indexPath, item: item.emoji)
                }
            }
        }
    }
    ...
    func applyInitialSnapshots(){
        ...
        let recentItems = Emoji.Category.recents.emojis.map { Item(title: "", emoji: $0, hasChildren: false) }
        ...
        for category in Emoji.Category.allCases where category != .recents {
            ...
            let allSnapshotItems = category.emojis.map { Item(title: "", emoji: $0, hasChildren: false) }
            allSnapshot.append(allSnapshotItems)
            
            // setup our parent/child relations
            let rootItem = Item(title: String(describing: category), emoji: Emoji(text: "", title: "", category: .activities), hasChildren: true)
            outlineSnapshot.append([rootItem])
            let outlineItems = category.emojis.map { Item(title: "", emoji: $0, hasChildren: false) }
...
extension CustomEmojiExplorerViewController: UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        print("emoji> ", emoji)
        let detailViewController = EmojiDetailViewController(emoji: emoji)
